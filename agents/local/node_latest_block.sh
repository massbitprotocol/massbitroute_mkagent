...
if [ "$_type" == "mbr-api" ]; then
	_n11=$(curl -sk --location --request POST 'https://dot.getblock.io/mainnet/' --header 'x-api-key: 6c4ddad0-7646-403e-9c10-744f91d37ccf' --header 'Content-Type: application/json' --data-raw '{"jsonrpc":"2.0","method":"chain_getBlock","params": [],"id": 1}' | jq .result.block.header.number | sed 's/\"//g' | sed 's/^0x//g')
	if [ -n "$_n11" ]; then
		_n1=$((16#$_n11))
		if [ $_n1 -gt 0 ]; then
