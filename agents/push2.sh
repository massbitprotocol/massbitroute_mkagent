...
_kill() {
	...
	kill $(ps -ef | grep -i push.sh | grep -v grep | awk '{print $2}')
	kill $(ps -ef | grep -i push.py | grep -v grep | awk '{print $2}')
}
...
*)
	...
	while true; do
		bash $CHECK_MK_AGENT >/tmp/checkmk
		timeout 10 curl -X POST -d "@/tmp/checkmk" $URL/push/$TOKEN
		sleep 30
	done
